...
public final class U {
    ...
    private static final String IMAGE = "(?i)^(.*)\\.(ico|gif|jpeg|jpg|bmp|png)$";
    ...
    public static double randomDouble() {
        return new BigDecimal(RANDOM.nextDouble()).setScale(6, BigDecimal.ROUND_HALF_EVEN).doubleValue();
    }

    /** 生成指定数值到 0 之间的随机数(数字) */
    public static int withinRandom(int num) {
    ...
    public static <E extends Enum> E toEnum(Class<E> clazz, Object obj) {
        if (isNotBlank(obj) && isNotBlank(clazz)) {
     ...
     * 显示用的 sku 描述.
     *
     * {"颜色":"白色","机身内存":"64GB"}  ==> 颜色:白色|机身内存:64GB
     */
    public static String skuDesc(String skuProperties) {
        if (U.isBlank(skuProperties)) {
            return EMPTY;
        }
        return skuProperties.replace("{", EMPTY).replace("}", EMPTY).replace("\"", EMPTY).replace(",", "|");
    }
    ...
    public static void assertLength(String str, int min, int max, String name) {
        if (!lengthBorder(str, min, max)) {
            assertException(String.format("%s长度在 %s 到 %s 位之间", name, min, max));
        }
    }
