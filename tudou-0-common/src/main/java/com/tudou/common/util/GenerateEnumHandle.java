...
public class GenerateEnumHandle {
    ...
    public static void generateEnum(Class clazz, String packageName, String moduleName) {
        String packageAndModule = packageName + "." + moduleName.replace('-', '.');

        String packName = packageAndModule + ".enums";
        URL url = clazz.getClassLoader().getSystemResource(packName.replace(".", "/"));
        if (url == null) {
            ...
            for (File file : packageParent.listFiles()) {
                ...
                try {
                    ...
                    if (aClass != null && aClass.isEnum()) {
                        ...
                        String parent = clazz.getClassLoader().getSystemResource("").getPath()
