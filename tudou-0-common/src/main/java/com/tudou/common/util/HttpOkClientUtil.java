...
public class HttpOkClientUtil {
    ...
    public static String postWithHeader(String url, Map<String, Object> header, Map<String, Object> params) {
        Request.Builder builder = new Request.Builder().post(RequestBody.create(JSON, com.alibaba.fastjson.JSON.toJSONString(params)));
        for (Map.Entry<String, Object> entry : header.entrySet()) {
            builder.addHeader(entry.getKey(), entry.getValue().toString());
        }
        return handle(url, builder);
    ...
    public static String postWithJson(String url, Map<String, Object> params) {
        return handle(url, new Request.Builder().post(RequestBody.create(JSON, com.alibaba.fastjson.JSON.toJSONString(params))));
    }
    ...
    public static void download(String url, String file) throws IOException {
        ...
        if (response != null) Files.write(response.bytes(), new File(file));
    }
        ...
        if (response != null) {
            try {
                ...
                if (LogUtil.ROOT_LOG.isWarnEnabled())
                    LogUtil.ROOT_LOG.warn(String.format("response (%s) exception", response.toString()), e);
            }
        }
    ...
    private static ResponseBody response(String url, Request.Builder builder) {
        ...
        try {
            ...
            if (LogUtil.ROOT_LOG.isWarnEnabled())
                LogUtil.ROOT_LOG.warn(String.format("request (%s) exception", request.toString()), e);
        }
        return null;
    }
