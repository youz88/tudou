...
public final class Encrypt {
    ...
    public static String aesEncode(String data) {
        if (data == null) throw new RuntimeException("空值无法加密");
        if (AES_SECRET.length != 16) throw new RuntimeException("密钥必须是 16 位");
        try {
    ...
    public static String aesDecode(String data) {
        if (data == null || data.trim().length() == 0) throw new RuntimeException("空值无法解密");
        if (AES_SECRET.length != 16) throw new RuntimeException("密钥必须是 16 位");
        try {
            // 二进制转成十六进制
            byte[] bt = data.getBytes(StandardCharsets.UTF_8);
            if ((bt.length % 2) != 0) throw new RuntimeException("非偶数位的值无法解密");
    ...
    public static String toMd5File(String file) {
        try (FileInputStream in = new FileInputStream(file)) {
            ...
            if (LogUtil.ROOT_LOG.isDebugEnabled())
                LogUtil.ROOT_LOG.debug("无法给文件(" + file + ")生成 md5", e);
